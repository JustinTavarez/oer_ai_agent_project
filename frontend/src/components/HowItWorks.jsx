  ...
  return (
    <section id="how-it-works" className="px-6 py-24 md:px-10">
      <div className="mx-auto max-w-6xl">
        ...
        >
          <p className="text-sm font-medium tracking-[0.16em] text-brand-light">HOW IT WORKS</p>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {steps.map((step, index) => (
            <motion.div
              key={step.title}
              className="rounded-3xl border border-white/12 bg-gradient-to-b from-white/[0.08] to-white/[0.02] p-7"
              initial={{ opacity: 0, x: -24 }}
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.55, delay: index * 0.16, ease: "easeOut" }}
            >
              <div className="mb-5 flex h-11 w-11 items-center justify-center rounded-full bg-brand/25 text-sm font-semibold text-brand-light">
                {index + 1}
              </div>
